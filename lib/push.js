...
export const sendPushToUser = async (env, userId, message) => {
  ...
  const body = normalizeString(message?.notification?.body || data.body || '有新的后端消息待领取', 500)
...
export const notifyOutboxAvailable = async (env, userId, item = {}) => {
  try {
    ...
    return await sendPushToUser(env, userId, {
      data: {
        title: payload.title || payload.senderName || 'Lucky幸运机',
        body: payload.text || payload.body || payload.content || '有新的后端消息待领取',
        outbox: '1',
        outboxId: item.id || '',
        outboxType: type,
