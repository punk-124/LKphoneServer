...
// .wrangler/tmp/bundle-BGr99C/checked-fetch.js
...
// .wrangler/tmp/bundle-BGr99C/strip-cf-connecting-ip-header.js
...
app.post("/resources", async (c) => {
  const { user_id, category, title, content } = await c.req.json();
  ...
  try {
    const result = await c.env.DB.prepare(`
      INSERT INTO resources (user_id, category, title, content) 
      VALUES (?, ?, ?, ?)
    `).bind(user_id, category, title, content).run();
...
app.post("/sync", async (c) => {
  const { user_id, key, value } = await c.req.json();
  await c.env.DB.prepare(`
    INSERT OR IGNORE INTO users (user_id, username) 
    VALUES (?, ?)
  `).bind(user_id, user_id).run();
...
// .wrangler/tmp/bundle-BGr99C/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = LKphoneServer_default;
...
// .wrangler/tmp/bundle-BGr99C/middleware-loader.entry.ts
